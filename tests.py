...
class Tests:
	...
	@staticmethod
	def count_error(Xprim, X, size):  # counting an error with fraction matrix result usage
		error_summary_X = 0
		error_summary_Xprim = 0
		if isinstance(X[0], Fraction):
			for i in range(size):
				error_summary_X += X[i]
				error_summary_Xprim += Xprim[i]
				size_fract = Fraction(size)
			return math.fabs((error_summary_X - error_summary_Xprim) / size_fract)
		else:
			error_summary_X = abs(sum(X))
			error_summary_Xprim = abs(sum(Xprim))
			return math.fabs((error_summary_X - error_summary_Xprim)/size)
	...
	def test_1(self):  # dla odpowiedzenia na H1, H2, H3
		...
		plt.bar([1, 2, 3], [tf_error_sum_method1, tf_error_sum_method2, tf_error_sum_method3], label="dla dokladnosci pojedynczej precyzji")
		plt.bar([1, 2, 3], [td_error_sum_method1, td_error_sum_method2, td_error_sum_method3], label="dla dokladnosci podwojnej precyzji")
		# plt.scatter([tc_error_sum_method1, tc_error_sum_method2, tc_error_sum_method3], [1, 2, 3],label="dla typu fraction", marker=".")
		...
		plt.scatter([1, 2, 3], [tf_time_sum_method1, tf_time_sum_method2, tf_time_sum_method3], label="dla dokladnosci pojedynczej precyzji", marker=".")
		plt.scatter([1, 2, 3], [td_time_sum_method1, td_time_sum_method2, td_time_sum_method3], label="dla dokladnosci podwojnej precyzji", marker=".")
		# plt.scatter([tc_time_sum_method1, tc_time_sum_method2, tc_time_sum_method3], [1, 2, 3], label="dla dokladnosci wlasnego typu", marker=".")
		plt.xlabel('Numery metod 1-G, 2-PG, 3-FG')
	...
	def test_2(self):  # dla odpowiedzi na Q1
			...
			plt.ylabel('Sredni blad dokladnosci')
			plt.title(
	...
	def test_3(self):  # dla odpowiedzi na Q2
		...
		for size in sizes_array:  # dla metody G-gauss
			...
			for i in range(repeats):
				...
				# matrix.convert_typess(Fraction)
				# start_time = time.time()
				# self.tc_result = matrix.gauss()
				# elapsed_time = time.time() - start_time
				# results_Q2_tf_sum += elapsed_time
	...
	def test_4(self):  # dla odpowiedzenia na H1, H2, H3
		...
		#             label="dla dokladnosci wlasnego typu",
		#             marker=".")
		plt.xlabel('Numery metod 1-G, 2-PG, 3-FG')
		plt.ylabel('Czas potrzbeny na wyliczenie metod')
		plt.title('Wykres pokazujacy czas obliczania\n w zaleznosci od metody i rodzaju dokladnosci danych')
